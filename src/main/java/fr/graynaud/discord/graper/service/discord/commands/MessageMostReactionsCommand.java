...
@Component
public class MessageMostReactionsCommand extends FilteredCommand implements SlashCommand {

    private static final int NB_WORDS = 5;

    private final EsMessageService esMessageService;

    public MessageMostReactionsCommand(EsMessageService esMessageService) {
        this.esMessageService = esMessageService;
    }

    @Override
    public String getName() {
        return "messagemostreaction";
    }
    ...
    public Mono<Void> handle(ChatInputInteractionEvent event) {
        ...
        return event.deferReply()
                    .then(this.esMessageService.searchMessageMostReactions(event.getInteraction().getGuildId().get().asString(), filter, NB_WORDS)
    ...
    public ApplicationCommandRequest getRequest() {
        return ApplicationCommandRequest.builder()
                                        .name(getName())
                                        .nameLocalizationsOrNull(Map.of("fr", "message_plus_reaction"))
                                        .description("Remonte le top " + NB_WORDS + " des messages avec le plus de réactions.")
                                        .options(getOptions())
                                        .build();
    }

    private String phrase(Filter filter) {
        StringBuilder sb = new StringBuilder("Les " + NB_WORDS + " messages avec le plus de réactions ");
    ...
    private Field mapField(EsMessage message) {
        return Field.of("", "Écrit par <@" + message.getAuthorId() + "> le " +
                            ...
                            message.getChannelId() + "> avec " + message.getNbReactions() + " réactions :\nhttps://discord.com/channels/" +
